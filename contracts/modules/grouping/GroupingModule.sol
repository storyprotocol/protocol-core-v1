    ...
    UUPSUpgradeable
{
    ...
    IGroupIPAssetRegistry public immutable
    ...
    constructor(
        ...
        if (disputeModule == address(0)) revert Errors.GroupingModule__ZeroDisputeModule();
    ...
    function mintLicenseTokens(
        ...
        if (amount == 0) {
            revert Errors.GroupingModule__MintAmountZero();
        }
        if (receiver == address(0)) {
            revert Errors.GroupingModule__ReceiverZeroAddress();
        }
        if (!IP_ACCOUNT_REGISTRY.isIpAccount(licensorIpId)) {
            revert Errors.GroupingModule__LicensorIpNotRegistered();
        }
        _verifyIpNotDisputed(licensorIpId);
        Grouping.GroupingConfig memory lsc = LICENSE_REGISTRY.verifyMintLicenseToken(
            licensorIpId,
            licenseTemplate,
            licenseTermsId,
        ...
        if (lsc.licensingHook != address(0)) {
            mintingFeeByHook = IGroupingHook(lsc.licensingHook).beforeMintLicenseTokens(
                msg.sender,
                licensorIpId,
                licenseTemplate,
                licenseTermsId,
        ...
        if (!ILicenseTemplate(licenseTemplate).verifyMintLicenseToken(licenseTermsId, receiver, licensorIpId, amount)) {
            revert Errors.GroupingModule__LicenseDenyMintLicenseToken(licenseTemplate, licenseTermsId, licensorIpId);
        }
    ...
    function registerDerivative(
        ...
        if (parentIpIds.length != licenseTermsIds.length) {
            revert Errors.GroupingModule__LicenseTermsLengthMismatch(parentIpIds.length, licenseTermsIds.length);
        }
        if (parentIpIds.length == 0) {
            revert Errors.GroupingModule__NoParentIp();
        }
        ...
        if (!lct.verifyRegisterDerivativeForAllParents(childIpId, parentIpIds, licenseTermsIds, childIpOwner)) {
            revert Errors.GroupingModule__LicenseNotCompatibleForDerivative(childIpId);
        }
    ...
    function registerDerivativeWithLicenseTokens(
        ...
        if (licenseTokenIds.length == 0) {
            revert Errors.GroupingModule__NoLicenseToken();
        }
        ...
        ILicenseTemplate lct = ILicenseTemplate(licenseTemplate);
        if (!lct.verifyRegisterDerivativeForAllParents(childIpId, parentIpIds, licenseTermsIds, childIpOwner)) {
            revert Errors.GroupingModule__LicenseTokenNotCompatibleForDerivative(childIpId, licenseTokenIds);
        }
        ...
        for (uint256 i = 0; i < parentIpIds.length; i++) {
            ...
            if (i == 0) {
                ...
                revert Errors.GroupingModule__IncompatibleRoyaltyPolicy(royaltyPolicy, commonRoyaltyPolicy);
            }
        }
    ...
    function setGroupingConfig(
        address ipId,
        address licenseTemplate,
        uint256 licenseTermsId,
        Grouping.GroupingConfig memory licensingConfig
    ) external verifyPermission(ipId) whenNotPaused {
        if (
            licensingConfig.licensingHook != address(0) &&
            (!licensingConfig.licensingHook.supportsInterface(type(IGroupingHook).interfaceId) ||
                !MODULE_REGISTRY.isRegistered(licensingConfig.licensingHook))
        ) {
            revert Errors.GroupingModule__InvalidGroupingHook(licensingConfig.licensingHook);
        }
        if (licenseTemplate == address(0) && licenseTermsId == 0) {
            LICENSE_REGISTRY.setGroupingConfigForIp(ipId, licensingConfig);
        } else if (licenseTemplate != address(0) && licenseTermsId != 0) {
            LICENSE_REGISTRY.setGroupingConfigForLicense(ipId, licenseTemplate, licenseTermsId, licensingConfig);
        } else {
            revert Errors.GroupingModule__InvalidLicenseTermsId(licenseTemplate, licenseTermsId);
        }
    }
    ...
    function _payMintingFeeForAllParentIps(
        ...
        for (uint256 i = 0; i < parentIpIds.length; i++) {
            (address royaltyPolicy, bytes memory royaltyData) = _executeGroupingHookAndPayMintingFee(
                childIpId,
                parentIpIds[i],
                licenseTemplate,
                licenseTermsIds[i],
    ...
    function _executeGroupingHookAndPayMintingFee(
        address childIpId,
        address parentIpId,
        address licenseTemplate,
        uint256 licenseTermsId,
        ...
        Grouping.GroupingConfig memory lsc = LICENSE_REGISTRY.getGroupingConfig(
            parentIpId,
            licenseTemplate,
            licenseTermsId
        );
        ...
        uint256 mintingFeeByHook = 0;
        if (lsc.licensingHook != address(0)) {
            mintingFeeByHook = IGroupingHook(lsc.licensingHook).beforeRegisterDerivative(
                msg.sender,
                childIpId,
                parentIpId,
                licenseTemplate,
    ...
    function _verifyIpNotDisputed(address ipId) private view {
        if (DISPUTE_MODULE.isIpTagged(ipId)) {
            revert Errors.GroupingModule__DisputedIpId();
        }
    }
