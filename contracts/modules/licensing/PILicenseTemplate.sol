...
import { PILMetadataRenderer } from "./PILMetadataRenderer.sol";
    ...
    UUPSUpgradeable
{
    ...
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    PILMetadataRenderer public immutable METADATA_RENDERER;
    ...
    constructor(
        ...
        METADATA_RENDERER = PILMetadataRenderer(metadataRenderer);
    ...
    function getEarlierExpireTime(
        ...
        uint40 expireTime = _getExpireTime(licenseTermsIds[0], start);
        for (uint256 i = 1; i < licenseTermsIds.length; i++) {
            uint40 newExpireTime = _getExpireTime(licenseTermsIds[i], start);
    ...
    function toJson(uint32 licenseTermsId) public view returns (string memory) {
        PILTerms memory terms = _getPILicenseTemplateStorage().licenseTerms[licenseTermsId];
        return METADATA_RENDERER.toJson(terms);
    }

    /// @dev Encodes the commercial traits of PIL policy into a JSON string for OpenSea
    function _policyCommercialTraitsToJson(PILTerms memory terms) internal view returns (string memory) {
        return METADATA_RENDERER.policyCommercialTraitsToJson(terms);
    }

    /// @dev Encodes the derivative traits of PILTerm into a JSON string for OpenSea
    function _policyDerivativeTraitsToJson(PILTerms memory terms) internal view returns (string memory) {
        return METADATA_RENDERER.policyDerivativeTraitsToJson(terms);
    }
    ...
    function _getExpireTime(uint32 licenseTermsId, uint40 start) internal view returns (uint40) {
        PILTerms memory terms = _getPILicenseTemplateStorage().licenseTerms[licenseTermsId];
