...
/// @dev [CAUTION]
///      The LRP (Limited Royalty Percentage) royalty policy allows each remixed IP to receive a percentage of the revenue generated by its direct derivatives.
///      However, it is important for external developers to understand the potential dilution of royalties as more derivatives are created between two IPs.
...
///      Creator 2 - Registers IP3 as a derivative of IP2. Then Creator 2 decides to promote IP3 commercially in the market.
///      In this scenario, the earnings for Creator 1 are diluted because they will only receive 10% of the 20% royalties from IP3,
///      resulting in an effective royalty of 2%. If Creator 2 had chosen to promote IP2 instead, Creator 1 would have earned 10% directly, avoiding this dilution.
///      This lack of control over which IP is promoted commercially means that Creator 1 is exposed to significant dilution risk under the LRP royalty policy.
///
///      In contrast, the LAP (Limited Absolute Percentage) royalty policy enforces a fixed percentage on every descendant IP, protecting the original creator from dilution.
///
///      External developers considering the use of the LRP royalty policy should be aware of the potential for royalty dilution and consider measures to prevent/mitigate
///      the dilution risk or whether the LRP royalty policy is the right policy for their use case.
