...
/// @title ModuleRegistry_V2
/// @notice V2 of the ModuleRegistry contract with additional functionality
/// @custom:oz-upgrades-from contracts/registries/ModuleRegistry.sol
contract ModuleRegistry_V2 is IModuleRegistry, AccessManagedUpgradeable, UUPSUpgradeable {
    using Strings for *;
    ...
    /// @notice Event emitted when foo is called
    event FooCalled(address indexed caller, string message);
    ...
    function foo(string memory message) external {
        emit FooCalled(msg.sender, message);
    }
