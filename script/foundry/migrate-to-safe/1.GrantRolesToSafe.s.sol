...
contract GrantRolesToSafe is Script, AccessManagerOperations {
    ...
    function _generate() internal virtual override {
        ...
        if (block.chainid == 1514) {
            ...
            bytes memory _txData1 = abi.encodeWithSelector(
                AccessManager.grantRole.selector,
                ADMIN_ROLE_ID,
                governanceSafeMultisig,
                delay
            );
            _generateRegularTx(from[0], _txData1);

            bytes memory _txData2 = abi.encodeWithSelector(
                AccessManager.grantRole.selector,
                UPGRADER_ROLE_ID,
                governanceSafeMultisig,
                delay
            );
            _generateRegularTx(from[0], _txData2);

            bytes memory _txData3 = abi.encodeWithSelector(
                AccessManager.grantRole.selector,
                PAUSE_ROLE_ID,
                governanceSafeMultisig,
                0
            );
            _generateRegularTx(from[0], _txData3);

            bytes memory _txData4 = abi.encodeWithSelector(
                AccessManager.grantRole.selector,
                GUARDIAN_ROLE_ID,
                securityCouncilSafeMultisig,
                delay
            );
            _generateRegularTx(from[0], _txData4);
        }
    }
}