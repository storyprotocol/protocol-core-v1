...
contract TestRoyaltyPrecompile is BaseTest {
    function setUp() public override {
        ...
        address newRoyaltyPolicyLAPImpl = address(new RoyaltyPolicyLAP(address(royaltyModule), address(ipGraphACL)));

        address newRoyaltyPolicyLRPImpl = address(
        ...
        UUPSUpgradeable(royaltyModuleProxy).upgradeToAndCall(
            address(newRoyaltyModuleImpl),
            abi.encodeCall(RoyaltyModule.initialize, (address(protocolAccessManager), uint256(15)))
        );
        vm.stopPrank();
